    ...
    elseif strcmp(type, 'zeros')
        ...
        for i=floor(row_filter/2)+1:row_image+floor(row_filter/2)
            for j=floor(column_filter/2)+1:column_image+floor(column_filter/2)
                output(i-floor(row_filter/2), j-floor(column_filter/2)) = sum(sum (filter.*new_image(i-floor(row_filter/2):i+floor(row_filter/2), j-floor(column_filter/2):j+floor(column_filter/2))));
            end
        end
